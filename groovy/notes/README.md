...
- оператор индекса `[]`:
  ...
  * поддерживает перчисления (`assert letters[1, 3] == ['b', 'd']`) и диапазоны (`assert letters[2..4] == ['c', 'd', 'e']`)
...
- поля и свойства:
  * стандартные можификаторы доступа
...
- замыкания отличаются от лямбд в Jav8. Спец. переменные внутри замыкания:
...
- для искользования в `GString`: `${-> x}`, тогда строка будет вычисляться каждый раз при использовании, станет динамической
...
- множественное присваивание: `def (a, b, c) = [1, 2]`. Так же можно разваливать объекты, если у них перегружен оператор индеса (`.getAt()`/`.putAt()`): `def (la, lo) = coordinates`
...
- GPath - язык описания графа объектов. Из нудныз восторгов по его поводу понятно только, что можно искать имена методов: `this.class.methods.name.grep(~/.*Bar/)`
- приведение типов (`as`):
...
- преимуществами статическойй компиляции являются типобезопасность и более высокая производительность

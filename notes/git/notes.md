...
Для форматирования патчей, команда принимает все опции `[mp] diff`. Для показа коммитов принимает некоторые опции `[mp] log`, такие как `--[no-]abbrev-commit`, `--pretty[=<format>]`, `--oneline` и т.д. В мане к команде есть об использовании кодировок в путях файлов и контенте объектов.

## [pi] ls-tree

Просмотр содержимого дерева. Корректно разыменовывает любые treeish-объекты (теги, коммиты, ссылки). Вывод команды можно лимитировать через `<pattern>` (`-r` = recursive):
...
Это корректный входной формат для `git update-index --index-info --stdin`. Формат аналогичен `git cat-file -p` для объектов-деревьев.

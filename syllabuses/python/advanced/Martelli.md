...
Левый нулю (`[:n]` ~ `[0:n]`), а правый (`[n:]` ~ `[n:len(seq)]`) - длине последовательности. Таким образом, правый дефолтный индекс указывает на элемент за последним элементом последовательности

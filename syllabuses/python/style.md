...
Это не относится к аргументам с аннотациями: `def func(a: int = 0) -> int:`
...
В верхнем регистре, слова разделены подчеркиваниями (`MAX_HOLY_HANDGRENADE_COUNT`)
...
---
> Как говорит принцип YAGNI об именованных параметрах функций?
...
Через оператор `in` (`'hello' in d`) или получать без проверки, через `.get()` с дефолтным аргументом (`d.get('hello', 'some')`). Но не через `d.has_key('hello')`
...
---
> Почему запись `[print(x) for x in sequence]` плоха?
...
---
> Какой предпочтительный способ получения индекса при итерации?
...
Избегать: при импортах идет из присваивание, и это может приводить к непредсказуемому поведению
...
Google: (наоборот) В описательном (`"""Fetches from`), не в повелительном (~~`"""Fetch from`~~)
...
По Google style guide - 2 пробела после строки кода и 1 - после символа комментария: `if i & (i-1) == 0:  # True if ...`

---
> Какой предпочтительный способ собрать строку из частей в цикле?

Не использовать `+`/`+=`: т.к. строки неизменяемы, это приведет к созданию слишком большого числа временных объектов и квадратичной сложности вместо линейной. Лучше добавлять подстроки в список, а после цикла вызвать `''.join([...])`
...
---
> Должна ли ставится точка в конце первой строки в docstring?

Да, должна: `"""Return the pathname of the KOS root directory."""`

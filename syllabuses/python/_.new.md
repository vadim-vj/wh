...
Важно вести проверку до квадратного корня включительно (`int(i ** .5) + 1`) - напр., `25` делится на свой корень `5`, и если перебор до него не дойдет, число бует признано простым:

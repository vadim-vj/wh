...
- решения этих проблем делятся на три группы, три правильных выбора
  * алгоритма. Такой алгоритм не обязан загружать весь датасет в память
    ...
    - MapReduce, отображение-свертка. Сначала производит отображение набора значений на ключ (map), а потом объединение значений по этому ключу (reduce). Б*о*льшую часть работы на себя берут библиотеки Python, такие как Hadoopy, Octopy, Disco, Dumbo
  * структуры данных
    ...
    - хеш-таблицы также ускоряют выборку и также используются для индексов БД. Близкие родственники хранилищ "ключ-значение". Пишет, что ключи объединяются в какие-то *buckets* ("гнезда")
...
- пример 1, прогнозирование вредоносных урлов
  ...
  * т.о. для экономии памяти работаем сразу с разреженным данными - что-то там про формат SVMLite, который их по дефолту поддерживает. Кроме того, через библиотеку `tarfile` перебираем файлы в датасете, не распаковывая его
...
- пример, рекомендательная система
  ...
  * с гитхаба берут два файла: возможные ингредиенты в формате .txt и большую подборку рецептов в .json

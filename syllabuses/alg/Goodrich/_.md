...
- 01.11.2020
  ...
  * рекурсивный и нет методы обращения последовательности (4 варианта), их сложность
  * есть ли в последовательности пара, чье произведение нечетно
  * 3 варианта проверки, что все элементы последовательности уникальны; их сложность
  * генератор `n(n+1)`
  * реализация `random.shuffle()` через `random.randint()`
- 02.11.2020
  * сортировки пузырьком и вставками
...
См. [english_ruler.py](english_ruler.py)

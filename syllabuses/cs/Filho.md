...
* Качество балансировки дерева
  - чем больше высота дерева, тем больше требуется переходов по ссылкам (больше шагов, больше обращений к памяти)
...
* *Хеш-таблица (hash table)*
  ...
  - чем шире диапазон данных, выдаваемых хеш-функцией, тем меньше вероятность коллизии. Поэтому желательно чтобы в хеш-таблице оставалось незанятым по крайней мере 50 % пространства
...
* *Последовательный поиск*
  - проверка элементов один за одним, пока не будет найден нужный
  - сложность `O(n)`

* *Двоичный поиск*
  - работает только для сортированного списка
  - на каждом шаге отбрасывает половину пространства поиска
  - проверяет средний элемент сначала на равенство, а затем на знак "больше" - выбирает, какую половину оставить
  - сложность как и у бинарного дерева поиска - `O(log(n))`

Еще более быстрый поиск - по хеш-таблице. Он всегда занимает константное время, `O(1)`

## Графы

* *Поиск в графах*
  - два алгоритма поиска узла по ключу: *поиск в глубину* (DFS, depth first search) и *поиск в ширину* (BFS, breadth first search)
  - оба вида обхода используют поиск с возвратом - запоминают вершину в стеке/очереди, и потом к ней возвращаются
  - разница лишь в порядке перебора дочерних узлов: DFS запоминает их в стеке и поэтому перебирает их начиная с левого, в глубину, а BFS запоминает в очереди, и не переходит на следующий уровень, пока не обработает все на текущем
  - внешний цикл перебирает запомненные в коллекции узлы и берет первый; внутренний цикл добавляет в коллекцию дочерние узлы текущего
  - дополнительная отсечка узлов идет по вспомогательному множеству уже просмотренных узлов

* *Раскраска графов*, пример задачи
  - дана карта вышек сотовой связи. Смежные вышки, для предотвращения помех, должны работать на разных частотах. Есть 4 частоты на выбор. Какую частоту назначить каждой вышке?
  - сеть вышек моделируется графом
  - близко расположенные вышки (вызывающие помехи) соединяем ребром
  - нужно раскрасить граф так, чтобы смежные вершины не имели одинакового цвета (частоты)

* *Поиск путей в графе*
  - требование - поиск кратчайшего пути между двумя узлами ориентированного взвешенного графа
  - пример задачи - построение (навигатором) маршрута. Улицы с пробками могут представляться ребрами с более высокими весами
  - хорошим алгоритмом поиска кратчайшего пути является *алгоритм Дейкстры*
  - для больших графов может использоваться распараллеливание: алгоритм *двунаправленного поиска* запускает сразу два процесса, ищущих одновременно со стартовой и с конечной вершин

* *Алгоритм Дейкстры*
  - немного похож на BFS, но для запоминания следующих узлов использует очередь с приоритетом (им является вес ребер, приводящих в стартовую вершину)
  - благодаря приоритету, следующая исследуемая вершина всегда оказывается самой близкой к началу
  - в случае ребер с отрицательным весом в графе могут существовать *отрицательные циклы* - гамильтоновы циклы с отрицательным суммарным весом
  - на таких (отрицательных) циклах алгоритм Дейкстры зацикливается; таких ситуаций следует избегать
...
* *Задачи линейной оптимизации*
  - задачи, где целевой показатель и ограничения моделируются уравнениями первой степени
  - решаются *симплекс-методом*: он находит угловую точку замкнутой области - ту, где пересекаются прямые ограничений

* *Задача об умной меблировке*
  - шкаф X стоит 10 долларов, занимает 6 квадратных футов и содержит 8 кубических футов папок
  - шкаф Y стоит 20 долларов, занимает 8 квадратных футов и содержит 12 кубических футов папок
  - ограничения: 140 долларов, 72 квадратных футов площади офиса
  - целевой показатель: емкость хранения, должен быть максимизирован
  - обозначим: `x`/`y` — количество шкафов модели X/Y, целевая функция - `z = 8x + 12y`
  - ограничения: `10x + 20y ≤ 140` (по бюджету), `6x + 8y ≤ 72` (по площади), `x ≥ 0`, `y≥ 0` (количество не может быть отрицательным)
...
* *Реляционная модель*
  - данные разделены на *таблицы*, каждая запись в них - это *строка*, каждое свойство записи - *столбец* или *поле*
  - поля имеют тип хранимых данных и могут иметь некоторые ограничения, вроде обязательности данных, уникальности, и пр. Типы столбцов и ограничения вместе описывают *схему* таблицы
  - жесткость схемы, и невозможность добавить в таблицу строку, нарушающую эту схему, это один из недостатков РСУБД

* *Отношения*
  - для удаления дублирующейся информации из таблиц ее можно выносить в отдельные таблицы
  - вместо же вынесенной информации (напр., имени пользователя) можно использовать `id` - *первичный ключ*
  - ключевое поле, ссылающееся на другие таблицы, называется *внешним ключом*. Ссылки могут включать и промежуточную таблицу (если связь достаточно сложна)
  - процесс удаления дублирующейся информации из БД называется *нормализацией*, а сама БД без дублирования - *нормализованной*

* *Миграции*
  - при изменении схемы БД (схем любой из ее таблиц) часто пишут скрипт, который может автоматически делать эти изменения. Он называется *миграцией схемы*
  - как правило, этот скрипт может работать и в обратную сторону. Такое может быть полезно, например, при откате версии приложения
  - скрипт может не только менять схемы, но и преобразовывать существующие данные, добавлять новые / удалять старые
  - многие СУБД имеют готовые инструменты для поддержки миграций

* *SQL*
  - structured query language - "язык структурированных запросов". Язык запросов для реляционных СУБД, поддерживается практически всеми
  - в нем можно указывать, какие поля нужно получить, какие ограничения для выборки, есть возможность группировки и поддержка агрегатных функций. В условиях можно использовать булеву логику
  - поддержка отношений таблиц выполняется через несколько типов `JOIN`. Из-за возможного сильного увеличения числа записей при соединении таблиц, использовать его нужно осторожно: это одна из главных слабостей реляционной модели

* *Индексирование*
  - для ключей таблицы (напр., первичного) СУБД строит *индекс* - сбалансированное двоичное дерево поиска. Каждая запись соответствует узлу, ключи узлов - значения индексируемого поля `<!-- есть картинка -->`
  - поиск в сбалансированном BST занимает `O(log(n))`, поэтому и поиск по индексу быстрый
  - СУБД автоматически создает индекс для первичных ключей. Часто они автоматически создаются для уникальных полей (иначе при вставке проверка на уникальность потребует медленного просмотра всей таблицы). Для остальных полей его создание можно затребовать явно
  - если есть индекс, то сортировка по этому полю идет быстро. Иначе придется делать сортировку в памяти, и некоторые СУБД могут даже отказаться выполнять запрос с неиндексированным полем в `ORDER BY`
  - если требуется сортировка сразу по двум полям, то отдельные индексы для каждого поля бесполезны: всё равно потребуется сортировать по одному из полей вручную. Для таких случаев используют *комбинированные (объединенные) индексы*. Они не ускоряют поиск, но сильно упрощают сортировку по нескольким полям сразу
  - нежелательно делать индексы для всех подряд полей: это снижает производительность. Каждая вставка/удаление требуют обновления дерева поиска и (возможно) его балансировки, а это затратные операции. Кроме того, индексы занимают место на диске
  - большее количество индексов может иметь смысл в редко обновляющихся базах, где большинство операций - это чтение

* *Транзакции*
  - это список операций, которые должны быть выполнены *атомарно* - либо все, либо ни одной. Транзакция не может быть выполнена наполовину
  - нужны для поддержания целостности БД, непротиворечивости данных - чтобы в случае конкурирующих изменений или технических сбоев данные оставались корректными

## Нереляционная модель

* *Недостатки реляционной модели* с ростом приложения
  - увеличение количества таблиц
  - усложнение запросов - они становятся всё менее понятными
  - всё чаще приходится прибегать к `JOIN`-ам, требующим большого количества вычислений и создающим узкие места

* *Нереляционные СУБД*
  - не используют табличные связи (и поэтому не требуют соединения таблиц)
  - используют язык запросов, отличный от SQL, поэтому также называются *базами данных NoSQL*
  - *предполагают* возможность дублирования информации при необходимости

* *Документные хранилища*
  - самый известный тип NoSQL-баз. В них данные хранятся в том виде, в котором они нужны приложению
  - нет таблиц и строк, есть лишь записи, не имеющие фиксированной схемы (каждая может иметь собственное сочетание полей) `<!-- есть картинка -->`
  - эти записи называются *документами*. Они группируются в *коллекции*
  - хотя документы и имеют поле первичного ключа, операции `JOIN` в документных хранилищах неэффективны, а то и вовсе невозможны. Иногда это приводит к дублированию информации
  - как и в РСУБД, для первичных ключей документов создаются индексы. Их можно и явно запрашивать для других полей, по которым часто идет поиск или сортировка

* *Хранилища "ключ-значение"*
  - простейшая форма организованного хранения данных, в основном используется для кэширования
  - пример - URL (ключ) и HTML-содержимое страницы (значение)

* *Графовые базы данных*
  - записи хранятся в виде вершин, а связи - в виде ребер. Формат данных в вершинах нефиксированный (нет схемы)
  - пример - станции метро и остановки общественного транспорта. Граф-ориентированные запросы, типа поиска маршрутов, в такой базе будут просты, и будут поддерживаться нативно

* *Большие данные*
  - big data, ситуации обработки данных, которые чрезвычайно сложны с точки зрения объема, скорости или разнообразия
  - три "V": volume (объем), velocity (скорость) и variety (разнообразие). Иногда добавляют variability (переменчивость) и veracity (достоверность)
  - большие данные, из-за требований гибкости и большой общей сложности обработки, часто связаны с нереляционными СУБД

* *Сравнение SQL с NoSQL*
  - реляционные БД ориентированы на данные: максимизируют их структурирование и устраняют дублирование
  - нереляционные БД, напротив, ориентированы на применение: облегчают доступ к данным и их использование
  - базы данных NoSQL позволяют быстро и эффективно сохранять крупные, изменчивые и неструктурированные данные. Для программистов это часто проще, но и ответственность за дублирование данных (контроль, обновление) ложится на разработчика

## Распределенная модель

* *Ситуации, когда одного компьютера недостаточно для поддержания базы данных*
  - очень большой объем данных
  - очень большая нагрузка на процессор или на сеть
  - обработка жизненно важных данных - критичность к отказу системы

Для таких ситуаций существуют СУБД, способные работать на нескольких скоординированных компьютерах, образующие *распределенные базы данных* (distributed databases)

* *Репликация с одним ведущим*
  - Single-Master Replication, один (выделенный) компьютер получает все запросы на чтение/запись и делегирует их ведомым машинам
  - каждый ведомый компьютер содержит *реплику* (копию) базы данных. Операции записи синхронизирует ведущий компьютер
  - в случае отказа ведущего компьютера, остальные машины координируются и выбирают нового ведущего. Система не прекращает работу

* *Репликация с многочисленными ведущими*
  - Multi-Master Replication, используется в случае большого количества операций записи, когда один ведущий не справится
  - в этом случае все компьютеры являются ведущими. Для распределения запросов между ними используется балансировщик нагрузки
  - `<!-- есть картинка -->`

* *Фрагментирование*
  - Sharding, разделение базы на части и хранение частей (shard, фрагментов) на разных компьютерах
  - маршрутизатор запросов (query router) распределяет запросы между шардами
  - проблема здесь - фрагмент становится недоступен, если машина, на которой он хранится, вышла из строя. Чтобы этого избежать, каждый шард можно организовать в виде master-slave дублирования, совместить шардинг с репликацией
  - `<!-- есть картинка -->`

* *Непротиворечивость данных*
  - пример - покупка последнего билета двумя людьми на разных серверах. В результате, нужно либо поддерживать непротиворечивость (запрещать подобные запросы), либо откатывать некоторые запросы назад (возвращать деньги за билет одному из пользователей)
  - поддержание непротиворечивости данных влияет на производительность: часто для согласования данных требуется синхронизация всех серверов в кластере с блокировкой большого количества данных
  - компромиссом является *непротиворечивость в конечном счете* (eventual consistency) - данные будут целостными, но не обязательно сразу, а возможно через некоторое время. Некоторые запросы на запись могут быть отклонены, а некоторые запросы на чтение вернут устаревшую информацию

## Географическая модель

* *Географические информационные системы*
  - *ГИС* - особый формат баз данных, хранящих географическую информацию. Кроме того, многие СУБД предоставляют ГИС-расширения
  - они имеют специальные типы полей, вроде PointField (точка), LineField (прямая), PolygonField (многоугольник), и способны выполнять пространственные запросы в этих полях (вроде "города в пределах 10 миль от реки Миссисипи, упорядоченные по численности населения")
  - ГИС-приложения часто используются в ежедневной жизни, например, в GPS-навигаторах вроде Google Maps или Waze

## Форматы сериализации

Форматы данных, не зависящие от СУБД. Могут использоваться для экспорта данных в другую систему

- *SQL* - наиболее распространенный. Список команд, воспроизводящий базу данных. Многие СУБД имеют команды вроде `DUMP`/`RESTORE`
- *XML* - не зависит от СУБД и диалектов SQL, не самый практичный
- *JSON* - может представлять реляционные и нереляционные данные, интуитивно понятный, очень популярный. Имеет расширения, типа BSON (двоичный JSON; максимальная эффективность), JSON-LD - уклон в XML
- *CSV* - самый простой. Одна запись на строку, поля разделены запятыми. Не всегда подходит для сложных данных
...
- [Бинарный поиск](../tasks/binary-search)
- [Поиск в графе, в ширину и глубину](../tasks/graph-search)

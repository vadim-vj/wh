...
1. Задача о коммивояжере: найти кратчайший маршрут, позволяющий объехать `N` городов
...
> Два элементарных исхода (например, выпадение на кубике "4" и нечетного числа) являются *несовместимыми* (взаимно исключающими). Таким образом, единственная их связь - `xor` ("либо"). Вероятности таких событий *складываются*: "A либо B" ~ `A xor B` = `P = P1 + P2`
...
1. Задача о рюкзаке: предметы обладают весом и ценой, а рюкзак имеет максимальную вместимость по весу. Нужно упаковать его так, чтобы стоимость выбранных предметов была максимальна
   ...
   - т.к. идет перебор степенного множества, то сложность алгоритма экспоненциальная, `O(n^2)`
...
1. II. Задача о восьми ферзях: как разместить их на доске так, чтобы ни один из них не оказался под ударом других?
   ...
   - кроме того, полный перебор будет просчитывать заведомо ненужные комбинации, например, когда первые размешенные два ферзя бьют друг друга. Более эффективный подход состоит в проверке только приемлемых позиций
